...
const WEBAPP_URL = 'https://script.google.com/macros/s/AKfycby5GpmDU8RNmQtDzO2uZcd1v5T8aJFGjDLU-ouFGcHTALRczXKNumy4hSslgY8pHp1v/exec';

// Gmail / taken-configuratie
const TASK_EMAIL = 'nielbaaijens.taken@gmail.com';
const TASK_QUERY = `to:${TASK_EMAIL} -label:"Taken/Processed" -in:trash -in:spam`;
const MAX_THREADS_PER_RUN = 20;
const LABEL_OPEN = 'Taken/Open';
const LABEL_DONE = 'Taken/Done';
const LABEL_CANCEL = 'Taken/Geannuleerd';
const PROCESSED_LABEL_NAME = 'Taken/Processed';
...
function onOpen() {
  ...
  const automationMenu = ui.createMenu('Automatisering')
    ...
    .addSubMenu(
      ui.createMenu('Dagelijkse autosync')
...
function syncRow(sheet, row, headers, options) {
  ...
  const gmailThreadId = headers[HEADER.gmailThreadId]
    ? getCell(sheet, row, headers[HEADER.gmailThreadId])
    : '';

  updateGmailForRow_(gmailThreadId, isDone, isCancelled);
  if (shouldDelete) {
    if (existingEventId) {
      try {
  ...
  if (!omschrijving || !deadlineVal) {
    return 'skipped';
  }

  const title = buildTitle(categorie, omschrijving, toegewezen);
  const eventIdForDescription = existingEventId || '';
  ...
  if (!ev) {
    ...
    setCell(sheet, row, headers[HEADER.eventId], newId);

    const descWithLinks = buildDescription({
      categorie,
      omschrijving,
      toegewezen,
      status,
    ...
    ev.setDescription(descWithLinks);
...
function buildDescription({
  ...
  if (eventId && WEBAPP_URL) {
    const doneUrl = `${WEBAPP_URL}?action=done&eventId=${encodeURIComponent(eventId)}`;
    const cancelUrl = `${WEBAPP_URL}?action=cancel&eventId=${encodeURIComponent(eventId)}`;

    parts.push('');
    parts.push('Status bijwerken via deze links:');
    parts.push(`Done: ${doneUrl}`);
    parts.push(`Cancel: ${cancelUrl}`);
  }
...
// --- WEBAPP: status updaten via link in agenda ---
...
function doGet(e) {
  ...
  if (!action || !rawEventId) {
    return HtmlService.createHtmlOutput('Ontbrekende parameters: action en/of eventId.');
  }

  const ss = SpreadsheetApp.getActive();
  const sheet = ss.getSheetByName(SHEET_NAME);
  if (!sheet) {
    return HtmlService.createHtmlOutput('Sheet "' + SHEET_NAME + '" niet gevonden.');
  }

  const headers = getHeaderMap(sheet);
  if (!headers) {
    return HtmlService.createHtmlOutput('Kolomkoppen niet gevonden of onvolledig.');
  }

  const eventIdCol = headers[HEADER.eventId];
  if (!eventIdCol) {
    return HtmlService.createHtmlOutput('Kolom "' + HEADER.eventId + '" niet gevonden.');
  }

  const lastRow = sheet.getLastRow();
  const values = sheet.getRange(2, eventIdCol, Math.max(0, lastRow - 1), 1).getValues();
  const normalizedTargetId = normalizeEventId(rawEventId);

  let foundRowIndex = -1;
  for (let i = 0; i < values.length; i++) {
    const cellVal = values[i][0];
    if (!cellVal) continue;
    const normalizedCellId = normalizeEventId(String(cellVal));
    if (normalizedCellId === normalizedTargetId) {
      foundRowIndex = i + 2;
      break;
    }
  }
  ...
  if (foundRowIndex === -1) {
    return HtmlService.createHtmlOutput('Geen rij gevonden voor dit eventId.');
  }

  const doneCol = headers[HEADER.doneFlag];
  const cancelCol = headers[HEADER.cancelledFlag];
  const gmailCol = headers[HEADER.gmailThreadId];
  ...
  if (action === 'done') {
    ...
    return HtmlService.createHtmlOutput('Ongeldige action: ' + action);
  }

  if (doneCol) {
    sheet.getRange(foundRowIndex, doneCol).setValue(isDone);
  }
  if (cancelCol) {
    sheet.getRange(foundRowIndex, cancelCol).setValue(isCancelled);
  }
  if (!isDone && doneCol) {
    sheet.getRange(foundRowIndex, doneCol).setValue(false);
  }
  if (!isCancelled && cancelCol) {
    sheet.getRange(foundRowIndex, cancelCol).setValue(false);
  }

  let gmailThreadId = '';
  if (gmailCol) {
    gmailThreadId = sheet.getRange(foundRowIndex, gmailCol).getValue();
  }
  updateGmailForRow_(gmailThreadId, isDone, isCancelled);

  return HtmlService.createHtmlOutput(
    'Status bijgewerkt naar: ' + (isDone ? 'DONE' : 'GEANNULEERD') +
    ' voor eventId: ' + rawEventId
  );
}

// Gmail: labels bijwerken + archiveren indien nodig
function updateGmailForRow_(gmailThreadId, isDone, isCancelled) {
  ...
  try {
    ...
    Logger.log('Kon thread niet ophalen: ' + gmailThreadId + ' (' + e + ')');
    return;
  }
  if (!thread) return;

  const openLabel = GmailApp.createLabel(LABEL_OPEN);
  const doneLabel = GmailApp.createLabel(LABEL_DONE);
  const cancelLabel = GmailApp.createLabel(LABEL_CANCEL);

  if (isDone || isCancelled) {
    openLabel.removeFromThread(thread);
  }

  if (isDone) {
    doneLabel.addToThread(thread);
    cancelLabel.removeFromThread(thread);
    thread.moveToArchive();
  } else if (isCancelled) {
    cancelLabel.addToThread(thread);
    doneLabel.removeFromThread(thread);
    thread.moveToArchive();
  } else {
...
function checkMailInbox() {
  ...
  threads.forEach(thread => {
    ...
    if (headers[HEADER.cancelledFlag]) {
    ...
    try {
      const syncColIndex = sheet.getRange(`${SYNC_STATUS_COLUMN}1`).getColumn();
      sheet.getRange(newRow, syncColIndex).clearContent();
    } catch (err) {
      Logger.log('Kon sync-status niet leegmaken: ' + err);
    }
...
function _debugWhoAmI() {
  Logger.log('Script draait als: ' + Session.getActiveUser().getEmail());
  Logger.log('Gmail-aliases: ' + GmailApp.getAliases().join(', '));
}
